...
import net.realdarkstudios.rdslib.screen.slot.CustomFurnaceFuelSlot;
...
public abstract class AbstractFurnaceLikeMenu extends AbstractContainerMenu {
    ...
    public AbstractFurnaceLikeMenu(MenuType<?> menuType, RecipeType<? extends AbstractFurnaceLikeRecipe> recipeType, int id, Inventory inv, FriendlyByteBuf extraData) {
        this(menuType, recipeType, id, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(4));
    }

    public AbstractFurnaceLikeMenu(MenuType<?> menuType, RecipeType<? extends AbstractFurnaceLikeRecipe> recipeType, int id, Inventory inv, BlockEntity entity, ContainerData data) {
        super(menuType, id);
        checkContainerSize(inv, 3);
        blockEntity = (AbstractFurnaceLikeBlockEntity) entity;
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(handler -> {
            ...
            this.addSlot(new CustomFurnaceFuelSlot(handler, this,1, 56, 53));
            this.addSlot(new CustomFurnaceOutputSlot(handler, inv.player, 2, 116, 35));
        });
    ...
    /**
     * Makes sure the value is between 0 and {@code progressArrowSize}
     */
    ...
    public ItemStack quickMoveStack(Player pPlayer, int pIndex) {
        ...
        if (pIndex < VANILLA_FIRST_SLOT_INDEX + VANILLA_SLOT_COUNT) {
            ...
            } else if (pIndex >= VANILLA_FIRST_SLOT_INDEX && !this.moveItemStackTo(sourceStack, VANILLA_FIRST_SLOT_INDEX + HOTBAR_SLOT_COUNT, VANILLA_SLOT_COUNT + TE_INVENTORY_SLOT_COUNT - 1, false)) {
                return ItemStack.EMPTY;
            }
    ...
    /**
     * Determines if the stack can be smelted
     */
    public boolean canSmelt(ItemStack pStack) {
        return this.level.getRecipeManager().getRecipeFor((RecipeType<AbstractFurnaceLikeRecipe>)this.recipeType, new SimpleContainer(pStack), this.level).isPresent();
    }

    /**
     * Determines if the stack is fuel
     */
    ...
    public boolean shouldMoveToInventory(int pSlotIndex) {
        return pSlotIndex != TE_INVENTORY_FIRST_SLOT_INDEX + 1;
    }
}