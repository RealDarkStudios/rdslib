...
public class RecipeGenHelper {
    ...
    public void addCustomFurnaceRecipes(Consumer<FinishedRecipe> pFinishedRecipeConsumer, FurnaceLikeSerializer<? extends Recipe> pSerializer) {
        int pCookingTime = pSerializer.defaultCookingTime;
        
        customSmeltingList(pFinishedRecipeConsumer, COAL_SMELTABLES, RecipeCategory.MISC, Items.COAL, 0.1F, pCookingTime, "coal", pSerializer);
        customSmeltingList(pFinishedRecipeConsumer, IRON_SMELTABLES, RecipeCategory.MISC, Items.IRON_INGOT, 0.7F, pCookingTime, "iron_ingot", pSerializer);
        customSmeltingList(pFinishedRecipeConsumer, COPPER_SMELTABLES, RecipeCategory.MISC, Items.COPPER_INGOT, 0.7F, pCookingTime, "copper_ingot", pSerializer);
        customSmeltingList(pFinishedRecipeConsumer, GOLD_SMELTABLES, RecipeCategory.MISC, Items.GOLD_INGOT, 1.0F, pCookingTime, "gold_ingot", pSerializer);
        customSmeltingList(pFinishedRecipeConsumer, DIAMOND_SMELTABLES, RecipeCategory.MISC, Items.DIAMOND, 1.0F, pCookingTime, "diamond", pSerializer);
        customSmeltingList(pFinishedRecipeConsumer, LAPIS_SMELTABLES, RecipeCategory.MISC, Items.LAPIS_LAZULI, 0.2F, pCookingTime, "lapis_lazuli", pSerializer);
        customSmeltingList(pFinishedRecipeConsumer, REDSTONE_SMELTABLES, RecipeCategory.REDSTONE, Items.REDSTONE, 0.7F, pCookingTime, "redstone", pSerializer);
        customSmeltingList(pFinishedRecipeConsumer, EMERALD_SMELTABLES, RecipeCategory.MISC, Items.EMERALD, 1.0F, pCookingTime, "emerald", pSerializer);
        terracottaRecipes(pFinishedRecipeConsumer, pSerializer);
        FurnaceLikeRecipeBuilder.customSmelting(Ingredient.of(ItemTags.SAND), RecipeCategory.BUILDING_BLOCKS, Blocks.GLASS.asItem(), 0.1F, pCookingTime, pSerializer).unlockedBy("has_sand", has(ItemTags.SAND)).save(pFinishedRecipeConsumer, new ResourceLocation(modid, getItemName(Blocks.GLASS) + "_from_nether_brick_smelting_" + getItemName(Blocks.SAND)));
    ...
    public void customSmeltingList(Consumer<FinishedRecipe> pFinishedRecipeConsumer, List<ItemLike> pIngredients, RecipeCategory pCategory, Item pResult, float pExperience, int pCookingTime, String pGroup, RecipeSerializer<? extends Recipe> pSerializer) {
        for(ItemLike itemlike : pIngredients) {
            FurnaceLikeRecipeBuilder.customSmelting(Ingredient.of(itemlike), pCategory, pResult, pExperience, pCookingTime, pSerializer).group(pGroup).unlockedBy(getHasName(itemlike), has(itemlike)).save(pFinishedRecipeConsumer, new ResourceLocation(modid, getItemName(pResult) + "_from_nether_brick_smelting_" + getItemName(itemlike)));
        }
    }

    public void terracottaRecipes(Consumer<FinishedRecipe> pFinishedRecipeConsumer, FurnaceLikeSerializer<? extends Recipe> pSerializer) {
    ...
    private String getCustomSmeltingRecipeName(ItemLike itemLike, RecipeSerializer<? extends Recipe> pSerializer) {
